...
struct gr_node * gr_add_node(struct graph *g, union scalar_u decor)
{
	...
	abort_unless(g && g->mem);

	n = mem_get(g->mem, sizeof(*n));
	if ( !n )
		return NULL;
	n->graph = g;
	...
	n->decor = decor;
	n->out.arr = mem_get(g->mem, sizeof(struct gr_edge*) * 
	...
	if ( !g->isbi ) {
		n->in.arr = mem_get(g->mem, sizeof(struct gr_edge*) * 
					    CAT_GRAPH_INIT_EDGE_LEN);
		if ( !n->in.arr ) {
			mem_free(g->mem, n->out.arr);
			mem_free(g->mem, n);
			return NULL;
		}
			 ...
			 struct gr_edge_arr *ea2)
{
	...
	if ( ea->fill == ea->len ) {
		...
		newarr = mem_resize(g->mem, ea->arr, 
                             ...
                             union scalar_u decor)
{
	...
	g = from->graph;
	abort_unless(g && g == to->graph && g->mem);

	edge = mem_get(g->mem, sizeof(*edge));
	if ( !edge )
		return NULL;
