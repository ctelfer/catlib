  ...
  while ( *fmt ) {
    ...
    case 'H':
      if ( ! nreps ) {
        if ( PSIZ_HALF > left )
          goto error;
        p = pack_half(p, (half)va_arg(ap, int), bigendian);
        left -= PSIZ_HALF;
      } else {
        if ( (nreps > PSIZ_HALF_OVERFLOW) || (PSIZ_HALF * nreps > left) )
          goto error;
    ...
    case 'W':
      if ( ! nreps ) {
        ...
        if ( (nreps > PSIZ_WORD_OVERFLOW) || (PSIZ_WORD * nreps > left) )
          goto error;
    ...
    case 'J':
      if ( ! nreps ) {
        ...
        if ( (nreps > PSIZ_JUMBO_OVERFLOW) || (PSIZ_JUMBO * nreps > left) )
          goto error;
...
size_t unpack(void *buf, size_t blen, const char *fmt, ...)
{
  ...
  while ( *fmt ) {
    ...
    switch ( *fmt ) {
    case 'b':
      if ( (nreps > PSIZ_BYTE_OVERFLOW) || (blen < PSIZ_BYTE * nreps) || 
...
size_t packlen(const char *fmt, ...)
{
  ...
  while ( *fmt ) {
    ...
    case 'b':
    case 'B':
      if ( (nreps > PSIZ_BYTE_OVERFLOW) || (nreps * PSIZ_BYTE > max) )
        goto error;
      max -= PSIZ_BYTE * nreps;
      break;
    ...
    case 'h':
    case 'H':
      if ( (nreps > PSIZ_HALF_OVERFLOW) || (nreps * PSIZ_HALF > max) )
        goto error;
      max -= PSIZ_HALF * nreps;
      break;
    ...
    case 'w':
    case 'W':
      if ( (nreps > PSIZ_WORD_OVERFLOW) || (nreps * PSIZ_WORD > max) )
        goto error;
      max -= PSIZ_WORD * nreps;
      break;
    ...
    case 'j':
    case 'J':
      if ( (nreps > PSIZ_JUMBO_OVERFLOW) || (nreps * PSIZ_JUMBO > max) )
        goto error;
