...
struct hnode *ht_snalloc(void *k, void *d, unsigned h, void *c)
{
	...
	case CAT_DT_RAW:
		...
		abort_unless(r1->len+sizeof(struct hnode)+sizeof(struct raw) >=
		       r1->len);
	...
	case CAT_DT_RAWCPY:
		...
		abort_unless(l + l2 >= l);
		abort_unless(l+l2+sizeof(struct hnode)+sizeof(struct raw) * 2 >=
		       l + l2);
...
struct anode *avl_snalloc(void *k, void *d, void *c)
{
	...
	case CAT_DT_RAW:
		...
		abort_unless(r1->len > 0);
		abort_unless(r1->data != NULL);
		abort_unless(r1->len+sizeof(struct raw)+sizeof(struct anode) >= 
		       r1->len);
	...
	case CAT_DT_RAWCPY:
		...
		abort_unless(l2 >= r2->len);
		abort_unless(l + l2 >= l);
		abort_unless(l+l2+sizeof(struct anode)+sizeof(struct raw)*2 >=
		       l + l2);
...
struct rbnode *rb_snalloc(void *k, void *d, void *c)
{
	...
	case CAT_DT_RAW:
		...
		abort_unless(l2 >= r2->len);
		abort_unless(l + l2 >= l);
		abort_unless(l+l2+sizeof(struct rbnode)+sizeof(struct raw)*2 >=
			l + l2);

		node = emalloc(sizeof(struct rbnode)+sizeof(struct raw)*2 +
			       l + l2);
		nk = r2 = (struct raw *)(node + 1);
		r2->len  = r1->len;
		r2->data = (char *)(r2 + 1);
