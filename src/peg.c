 ...
 * Expression <- Sequence Codeblock? (SLASH Sequence Codeblock?)*
 * Sequence <- Prefix*
...
static void peg_node_free(union peg_node_u *n)
{
	...
	case PEG_SEQUENCE:
		...
		peg_node_free((union peg_node_u *)n->seq.code);
...
static void *peg_node_new(int type, struct peg_cursor *pc, uint len,
			  uint nlines)
{
	union peg_node_u *n;
	n = calloc(sizeof(*n), 1);
		    ...
		    struct peg_id **idp)
{
	...
	id = peg_node_new(PEG_IDENTIFIER, pc, 1, 1);
	if ( id == NULL ) {
			 ...
			 struct peg_literal **litp)
{
	...
	lit = peg_node_new(PEG_LITERAL, pc, npc.pos - pc->pos, 
			   npc.line - pc->line + 1);
	if ( lit == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		return -1;
		       ...
		       struct peg_class **clsp)
{
	...
	cls = peg_node_new(PEG_CLASS, pc, 1, 1);
	if ( cls == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		return -1;
	}
			 ...
			 struct peg_primary **prip)
{
	...
	pri = peg_node_new(PEG_PRIMARY, pc, 0, 1);
	if ( pri == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		goto err;
	}
		     ...
		     struct peg_seq **seqp)
{
	...
	seq = peg_node_new(PEG_SEQUENCE, pc, 0, 1);
	if ( seq == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		return -1;
	}
		      ...
		      struct peg_code **codep)
{
	...
	code = peg_node_new(PEG_CODE, pc, 1, 1);
	if ( code == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		return -1;
	}
	...
	do {
		n = str_spn(STR(peg, &npc), cs_codestop);
		npc.pos += n;
		      ...
		      struct peg_expr **exprp)
{
	...
	rv = -1;
	expr = peg_node_new(PEG_EXPRESSION, pc, 1, 1);
	if ( expr == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		peg_node_free((union peg_node_u *)seq);
		return -1;
		     ...
		     struct peg_def **defp)
{
	...
	def = peg_node_new(PEG_DEFINITION, pc, npc.pos - pc->pos,
			   npc.line - pc->line + 1);
	if ( def == NULL ) {
		peg->err = PEG_ERR_NOMEM;
		goto err;
	...
	rbn = rb_lkup(&peg->def_table, &def->id->name, &d);
	if ( d == CRB_N ) {
...
static int resolve_node(struct peg_grammar *peg, union peg_node_u *n)
{
	int d;
	struct rbnode *rbn;
	...
	switch ( n->node.type ) {
	case PEG_DEFINITION:
		return resolve_node(peg, (union peg_node_u *)n->def.expr);

	case PEG_EXPRESSION:
		return resolve_node(peg, (union peg_node_u *)n->expr.seq);

	case PEG_SEQUENCE:
		if ( resolve_node(peg, (union peg_node_u *)n->seq.pri) < 0 )
			return -1;
		return resolve_node(peg, (union peg_node_u *)n->seq.next);

	case PEG_PRIMARY:
		if ( resolve_node(peg, n->pri.match) < 0 )
			return -1;
		return resolve_node(peg, (union peg_node_u *)n->pri.next);

	case PEG_IDENTIFIER:
		rbn = rb_lkup(&peg->def_table, &n->id.name, &d);
		if ( d != CRB_N ) {
			peg->err = PEG_ERR_UNDEF_LITERAL;
			peg->eloc = n->node.loc;
			return -1;
		}
		n->id.def = container(rbn, struct peg_def, rbe);
...
static int resolve_ids(struct peg_grammar *peg)
{
	...
	l_for_each(ln, &peg->def_list) {
		...
		if ( resolve_node(peg, (union peg_node_u *)def) < 0 )
			return -1;
	}
	return 0;
}
